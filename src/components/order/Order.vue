<template>
  <div>
    ...
    <el-card>
      ...
      <el-table border stripe :data="orderlist">
        ...
        <el-table-column
          ...
          prop="create_time"
          v-slot="scope"
        >{{scope.row.create_time | dataFormat}}</el-table-column>
    ...
    <!-- 物流的地址 -->
    <el-dialog title="物流实况" :visible.sync="progressVisible" width="50%">
      <el-timeline>
    <el-timeline-item
...
export default {
  ...
  methods: {
    ...
    async showProgressBox() {
      const { data: res } = await this.$http.get("kuaidi/1106975712662"); //804909574412544580
